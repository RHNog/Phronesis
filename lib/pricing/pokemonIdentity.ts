...
export const POKEMON_SET_ALIASES: Readonly<Record<string, string>> = {
  ...
  "heartgold and soulsilver promos": "hgss black star promos",
  ...
  "mcdonald s collection 2018 english": "mcdonald s collection 2018",
...
const POKEMON_SUBSET_SET_COMPATIBILITY: ReadonlyArray<{
  identities: readonly [string, string];
  collectorPrefix: string;
}> = [
  {
    identities: ["hidden fates", "hidden fates shiny vault"],
    collectorPrefix: "SV",
  },
  {
    identities: ["generations", "generations radiant collection"],
    collectorPrefix: "RC",
  },
];

const POKEMON_EXPLICIT_STRUCTURAL_SET_COMPATIBILITY: ReadonlyArray<
  readonly [string, string]
> = [
  ["double crisis", "magma gang vs aqua gang double crisis"],
];
...
export function pokemonSetLabelsStructurallyCompatible(
  ...
  const setPair = new Set([leftIdentity, rightIdentity]);
  if (
    setPair.size === 2 &&
    POKEMON_EXPLICIT_STRUCTURAL_SET_COMPATIBILITY.some((identities) =>
      identities.every((identity) => setPair.has(identity)),
    )
  ) {
    return true;
  }
  ...
  if (
    collectorIdentity &&
    POKEMON_SUBSET_SET_COMPATIBILITY.some(({ identities, collectorPrefix }) =>
      setPair.size === 2 &&
      identities.every((identity) => setPair.has(identity)) &&
      collectorIdentity.startsWith(collectorPrefix),
    )
  ) {
    return true;
  }
