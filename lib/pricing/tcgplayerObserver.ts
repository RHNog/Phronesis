...
export function archiveCompletedCatalogue(input: {
  ...
  if (existsSync(destination)) {
    if (hashFile(destination) !== input.sourceHash) {
      throw new Error(`Archived ${source.game} catalogue hash does not match its verified source.`);
    }
    return destination;
  }
  const temporary = join(directory, `.catalog_${source.game}.${process.pid}.${Date.now()}.tmp`);
  try {
    copyFileSync(input.catalogue.filePath, temporary);
    if (hashFile(temporary) !== input.sourceHash) {
      throw new Error(`Copied ${source.game} catalogue hash does not match its verified source.`);
    }
...
export function readCompletedCatalogues(toolRoot: string): CompletedCatalogue[] {
  ...
  if (typeof state.run_dir !== "string" || !state.steps || typeof state.steps !== "object") return [];
  const configuredRunDirectory = resolve(state.run_dir);
  const dataRoot = realpathSync(join(root, "data"));
  if (!existsSync(configuredRunDirectory) || lstatSync(configuredRunDirectory).isSymbolicLink()) {
    throw new Error("Pricing Update Tool run directory is missing or is a symlink.");
  }
  const runDirectory = realpathSync(configuredRunDirectory);
  if (!isInside(dataRoot, runDirectory)) {
    throw new Error("Pricing Update Tool run directory is outside its configured data root or is a symlink.");
  }

  const completed: CompletedCatalogue[] = [];
  for (const [categoryId, source] of Object.entries(tcgplayerCatalogSources) as Array<[TcgplayerCategoryId, (typeof tcgplayerCatalogSources)[TcgplayerCategoryId]]>) {
    const step = state.steps[`export_catalog::${source.game}`];
    if (step?.done !== true || typeof step.at !== "number" || !Number.isFinite(step.at)) continue;
    const filePath = join(runDirectory, `catalog_${source.game}.csv`);
    if (basename(filePath) !== `catalog_${source.game}.csv` || !existsSync(filePath)) {
      throw new Error(`Completed ${source.game} checkpoint has no readable catalogue file.`);
    }
    const file = lstatSync(filePath);
    if (!file.isFile() || file.isSymbolicLink() || file.size <= 256) {
      throw new Error(`Completed ${source.game} catalogue is not a stable regular file.`);
    }
    completed.push({
      categoryId,
    ...
    for (const catalogue of catalogues) {
      ...
      try {
        ...
        if (repository.hasImport(metadata)) {
          ...
          attempts.push({ categoryId: catalogue.categoryId, outcome: "ALREADY_IMPORTED", result });
          continue;
        }
        repository.recordSyncState({
          categoryId: catalogue.categoryId,
        ...
        const result = repository.importNormalizedRows(
          readTcgplayerCatalog(catalogue.filePath, catalogue.categoryId, catalogue.checkpointAt),
          metadata,
        );
        repository.recordSyncState({
          categoryId: catalogue.categoryId,
        ...
        attempts.push({ categoryId: catalogue.categoryId, outcome: result.status, result });
      } catch (error) {
        ...
        attempts.push({ categoryId: catalogue.categoryId, outcome: "FAILED", error: message });
      }
    }
  } finally {
    repository.close();
