...
export type EventAccessGrant = {
  id: string; eventId: string; eventName: string; workerLabel: string;
...
function assertEntitlements(values: readonly ModuleEntitlement[]): void {
  ...
  for (const value of values) {
    if (!ALLOWED_MODULES.includes(value.module as (typeof ALLOWED_MODULES)[number])) throw new Error("Event access is limited to Vendor Workspace, Event Ledger, Event Flip, Inventory, and Artwork Review.");
...
export class EventAccessRepository {
  ...
  createGrant(input: { workspaceId: string; eventId: string; workerLabel: string; entitlements: readonly ModuleEntitlement[]; durationHours: number; actorUserId: string }, at = new Date()): EventAccessGrant {
    ...
    const event = this.database.prepare("SELECT id, name, status FROM phronesis_purchase_event WHERE id=? AND workspace_id=?").get(input.eventId, input.workspaceId) as SqlRow | undefined;
    if (!event || event.status !== "ACTIVE") throw new Error("An active event is required.");
    ...
    this.database.prepare(`INSERT INTO phronesis_event_access_grant(
      id,workspace_id,event_id,worker_label,entitlements_json,code_hash,code_salt,status,expires_at,created_by_user_id,created_at
    ) VALUES(?,?,?,?,?,?,?,'ACTIVE',?,?,?)`).run(id, input.workspaceId, input.eventId, workerLabel, JSON.stringify(input.entitlements), scryptSync(normalized, salt, 32).toString("hex"), salt, expiresAt, input.actorUserId, at.toISOString());
    this.audit(input.workspaceId, input.actorUserId, "EVENT_ACCESS_CREATED", id, { eventId: input.eventId, workerLabel, entitlements: input.entitlements, expiresAt });
    return { id, eventId: input.eventId, eventName: String(event.name), workerLabel, entitlements: input.entitlements, status: "ACTIVE", expiresAt, createdAt: at.toISOString(), redeemedAt: null, revokedAt: null, code: plaintext };
  }

  listGrants(workspaceId: string, at = new Date()): EventAccessGrant[] {
    const rows = this.database.prepare(`SELECT g.*, e.name event_name, e.status event_status FROM phronesis_event_access_grant g JOIN phronesis_purchase_event e ON e.id=g.event_id WHERE g.workspace_id=? ORDER BY g.created_at DESC`).all(workspaceId) as SqlRow[];
  ...
  redeem(value: string, bucketHash: string, at = new Date()): { token: string; expiresAt: string; grant: EventAccessGrant } {
    ...
    if (candidate.length !== 16) { this.recordAttempt(bucketHash, false, at); throw new Error("Event access code is invalid or expired."); }
    const rows = this.database.prepare(`SELECT g.*, e.name event_name, e.status event_status FROM phronesis_event_access_grant g JOIN phronesis_purchase_event e ON e.id=g.event_id WHERE g.status='ACTIVE' AND g.expires_at>? AND e.status='ACTIVE'`).all(at.toISOString()) as SqlRow[];
    ...
    if (!row) { this.recordAttempt(bucketHash, false, at); throw new Error("Event access code is invalid or expired."); }
    ...
    try {
      ...
      if (Number(updated.changes) !== 1) throw new Error("Event access code is invalid or expired.");
  ...
  private mapGrant(row: SqlRow, at: Date): EventAccessGrant {
    const status = row.status === "REVOKED" ? "REVOKED" : row.event_status !== "ACTIVE" ? "EVENT_CLOSED" : new Date(String(row.expires_at)).getTime() <= at.getTime() ? "EXPIRED" : String(row.status) as "ACTIVE" | "REDEEMED";
    return { id: String(row.id), eventId: String(row.event_id), eventName: String(row.event_name), workerLabel: String(row.worker_label), entitlements: JSON.parse(String(row.entitlements_json)) as ModuleEntitlement[], status, expiresAt: String(row.expires_at), createdAt: String(row.created_at), redeemedAt: row.redeemed_at ? String(row.redeemed_at) : null, revokedAt: row.revoked_at ? String(row.revoked_at) : null };
  }
