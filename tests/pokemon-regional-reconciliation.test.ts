...
test("Pokémon regional identity remains exact across bounded catalogue conventions", () => {
  ...
  assert.equal(
    pokemonSetIdentity("SM Promos"),
    pokemonSetIdentity("Sun & Moon Promos"),
  );
  assert.equal(
    pokemonSetIdentity("SM Base Set"),
    pokemonSetIdentity("Sun & Moon"),
  );
  assert.equal(
    pokemonSetIdentity("HeartGold SoulSilver"),
    pokemonSetIdentity("HeartGold & SoulSilver"),
  );
  assert.equal(
    pokemonSetIdentity("HGSS Promos"),
    pokemonSetIdentity("HeartGold & SoulSilver Promos"),
  );
  assert.equal(
    pokemonSetIdentity("ME: Mega Evolution Promo"),
    pokemonSetIdentity("Mega Evolution Promos"),
  );
  assert.equal(
    pokemonSetIdentity("McDonald's Promos 2018"),
    pokemonSetIdentity("McDonald's Collection 2018 (English)"),
  );
  ...
  try {
    ...
    source.exec(`
      ...
      INSERT INTO ligapokemon_price VALUES
        ...
        ('gardevoir-source','Hidden Fates','HIF','Gardevoir-GX','SV75','Foil','NM','EN',16990,17990,18990,NULL,NULL,NULL),
    ...
    const gardevoir = regional.evidenceFor("pokemon-en", "gardevoir-sv75");
    assert.equal(gardevoir?.matchQuality, "EXACT");
    assert.equal(
      gardevoir?.matchMethod,
      "EXACT_POKEMON_STRUCTURAL_SET_NAME_COLLECTOR_FINISH_V1",
    );
    assert.equal(gardevoir?.consumerLowCentavos, 16990);
...
test("recurring and catalogue observers own Pokémon crosswalk continuity", () => {
  ...
  const importer = readFileSync(
    new URL("../scripts/import-tcgplayer-catalog.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.match(observer, /syncCompletedCatalogues/);
