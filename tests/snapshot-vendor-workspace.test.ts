...
test("snapshot selection reuses the existing purchase evaluation and offer ladder", () => {
  ...
  if (evaluation.status === "READY") {
    assert.ok(["BUY", "NEGOTIATE", "PASS"].includes(evaluation.decision.action));
    assert.ok(evaluation.negotiationLadder.openingOffer <= evaluation.negotiationLadder.targetOffer);
    assert.ok(evaluation.negotiationLadder.targetOffer <= evaluation.negotiationLadder.maximumBuyPrice);
...
test("artwork-first grouping collapses finish-only products but preserves alternate artwork", () => {
  ...
  const rainbow = { ...match, sku: "tcg:test-card-rainbow", name: "Test Card (0042) (Rainbow Foil)", variant: "Foil" };
  const borderless = { ...match, sku: "tcg:test-card-borderless", name: "Test Card (Borderless)", collectorNumber: "142", variant: "Foil" };
  const groups = groupSearchMatchesByArtwork([foil, match, rainbow, borderless]);
...
test("Vendor Workspace is desktop-first, keyboard-operable, and mobile-adaptive without a second decision engine", () => {
  const component = readFileSync(new URL("../features/vendor/components/SnapshotVendorWorkspace.tsx", import.meta.url), "utf8");
  const route = readFileSync(new URL("../app/vendor/page.tsx", import.meta.url), "utf8");
  assert.match(route, /SnapshotVendorWorkspace/);
  assert.doesNotMatch(route, /Open price lookup/);
  assert.match(component, /xl:grid-cols-\[minmax\(230px,0\.8fr\)_minmax\(280px,1fr\)_minmax\(300px,0\.95fr\)\]/);
  assert.match(component, /xl:sticky xl:top-5/);
  assert.match(component, /api\/pricing\/search/);
  assert.match(component, /api\/pricing\/artwork/);
  assert.match(component, /CardThumbnail/);
  ...
  const statusRoute = readFileSync(new URL("../app/api/pricing/status/route.ts", import.meta.url), "utf8");
