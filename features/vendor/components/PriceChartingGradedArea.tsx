...
function money(cents: number | null) { return cents === null ? "—" : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(cents / 100); }

export default function PriceChartingGradedArea({ match }: { match: SearchMatch }) {
  ...
  return <section aria-labelledby="graded-area-heading" className="mt-5 rounded-xl border border-violet-900/70 bg-violet-950/20 p-4">
    ...
    {evidence.map((item) => <details key={item.id} className="mt-3 rounded-lg border border-zinc-800 bg-zinc-950 p-3" open={evidence.length === 1}><summary className="cursor-pointer text-sm font-semibold text-zinc-100">{item.productName} <span className="font-normal text-zinc-500">· {item.catalogueName}</span></summary><div className="mt-3 grid grid-cols-3 gap-2">{Object.entries(item.prices).map(([grade, value]) => <div key={grade} className="rounded-md bg-zinc-900 p-2"><p className="text-[10px] text-zinc-500">{grade}</p><p className="mt-1 text-sm font-semibold tabular-nums text-violet-200">{money(value)}</p></div>)}</div><a href={item.sourceUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex min-h-11 items-center text-xs font-semibold text-violet-300">Verify on PriceCharting ↗</a></details>)}
    <p className="mt-3 text-[11px] leading-5 text-zinc-500">Candidate evidence remains separate until set, number, and variation are verified. It never overwrites TCGplayer pricing or artwork.</p>
  </section>;
}
