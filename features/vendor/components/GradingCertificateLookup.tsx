...
export default function GradingCertificateLookup({
  ...
  const [providerId, setProviderId] =
    useState<CertificateLookupProviderId>("PSA");
  const [certificateNumber, setCertificateNumber] = useState("");
  const [pending, setPending] = useState(false);
  const [result, setResult] = useState<LookupResponse | null>(null);
  const provider = certificateLookupProviders.find(
  ...
  async function lookup() {
    ...
    try {
      ...
      setResult({
        status: "ERROR",
        message: "Certificate lookup could not reach Phronesis.",
      });
    } finally {
      setPending(false);
    }
    ...
    <>
      ...
      >
        {pending
          ...
          : "Check certificate inside Phronesis"}
      </button>
      {result ? (
        <div
          role="status"
          className={`mt-3 rounded-lg border p-3 text-sm ${result.status === "VERIFIED" ? "border-emerald-700 bg-emerald-950/25 text-emerald-100" : "border-amber-800 bg-amber-950/25 text-amber-100"}`}
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {result.certificateNumber ? (
              <p className="font-mono text-xs">#{result.certificateNumber}</p>
            ) : null}
          </div>
          {result.title ? (
          ...
          {result.grade ? (
            <p className="mt-1">
              Grade {result.grade}
              {result.labelType ? ` · ${result.labelType}` : ""}
            </p>
          ) : null}
          {result.specificationNumber ? (
            <p className="mt-1 text-xs">
              PSA specification {result.specificationNumber}
            </p>
          ) : null}
          {result.message ? (
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <span className="text-xs font-medium text-zinc-500">
            PSA native · other graders registered
          </span>
        </div>
  ...
  return (
    <details className="rounded-xl border border-zinc-800 bg-zinc-900/70">
      ...
      <div className="border-t border-zinc-800 p-4">{content}</div>
    </details>
  );
}
