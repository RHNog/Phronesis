...
- `PHR-WORKFLOW-016` provides authenticated batch-folder intake, evidence review, durable session-state reconciliation, operator material confirmation, server-verified exact-condition price binding, buying-preset binding, and local offer drafts without purchase or inventory mutation. Its conservative operator path is privately active at tailnet-only `:9444`.
