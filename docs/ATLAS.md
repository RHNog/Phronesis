...
- `PHR-UX-020` keeps purchase corrections inside the canonical operator-owned open cart. Exact lines may change unit value and quantity; Bulk may change total value and optional approximate count. An operator can clear the complete owned active-event draft after confirmation and attach one private content-validated photo per line. Draft deletion retires its image; checkout retains photo metadata as immutable authorized receipt evidence while submitted receipt/ledger/Inventory/Case records remain immutable.
