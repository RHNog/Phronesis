...
- `getActiveRestrictedPublicOrigin` separates configured origins from an operational public route. Explicit `PHRONESIS_RESTRICTED_PUBLIC_MODE=ENABLED` selects the externally verified restricted-account origin; disabled mode derives the current browser origin.
- `PHR-TECH-016` is publicly live at `https://ramons-mac-studio.tailaa2d39.ts.net:10000/sign-up`. Visitors need neither Tailscale software nor tailnet membership. `access.phronesis.com` remains unresolved and is a later presentation/transport upgrade, not a prerequisite for remote account use.
...
- `PHR-UX-020` keeps purchase corrections inside the canonical operator-owned open cart. Exact lines may change unit value and quantity; Bulk may change total value and optional approximate count. Bulk picture intake appears before `Add Bulk` and binds the selected file to the one returned line; every line retains private replace/remove controls. An operator can clear the complete owned active-event draft after confirmation. Draft deletion retires its image; checkout retains photo metadata as immutable authorized receipt evidence while submitted receipt/ledger/Inventory/Case records remain immutable.
...
- `PHR-TECH-016` runs one loopback dual-policy gateway behind public Tailscale Funnel port 10000. Permanent-account entry paths and Better Auth session-cookie requests receive the restricted marker; event-worker entry retains the public-event marker. Both paths overwrite spoofable forwarding/ingress headers before the application makes its own signed-session and authorization decision.
- Restricted-public account requests require a permanent authenticated membership before optional compatibility or timed-worker evaluation and cannot transport-reach Settings, administration, developer, activation, or worker-login paths. Anonymous root traffic remains the public event entry, while signed approved users can reach only their assigned modules. A future `access.phronesis.com` tunnel may replace the visible `*.ts.net` origin after provider activation without weakening these controls.
