...
- WebApp shell rule: reciprocal sidebar controls stay inside `100dvh`, the tool list scrolls independently, the root canvas and scrollbar remain dark, Settings is a real destination, and app-only shortcuts are discoverable without intercepting editable fields or ordinary browser sessions.
...
- Verification: focused 8/8, full 425/425, TypeScript, full lint, Next.js 16.2.12 production build, desktop expanded/collapsed review, 390×844 review, approved-asset hash proof, live manifest/head inspection, 1280-pixel zero-overlap scroll geometry, installed-app `Cmd+B` and `G` then `D` interaction, accessible shortcut dialog, dark right-edge review, and zero browser console errors pass.
...
- Current accountable role: Product Owner runs replacement session `cb876a3c-c6da-42c3-8f8e-c587b387604a` only after cards are physically ready in PaperStream, then reviews any current Pokémon candidate with both sides visible. Corpus qualification and any auto-accept policy remain separate gated objectives.
- Current checkpoint: PaperStream **Release after scan** is enabled. Empty attempt `Poke Test #2` is durably `CANCELLED` with zero frames; replacement `Poke Test #3` is `CAPTURING` as Lightly Played / Holofoil. Phronesis now exposes an evidence-preserving confirmed Cancel control, cancels pending recognition work, and rejects late imports without claiming control of PaperStream. Full 425/425 tests, lint, Next.js production build with TypeScript, diff hygiene, live service restart, and operator UI verification pass.
