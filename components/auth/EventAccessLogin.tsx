...
export default function EventAccessLogin({ callbackURL = "/" }: { callbackURL?: string }) {
  ...
  return <form onSubmit={submit} className="mt-5 border-t border-zinc-800 pt-5">
    <h2 className="font-semibold text-white">Temporary worker</h2><p className="mt-1 text-sm text-zinc-400">No account required. Enter the code provided by the Phronesis owner.</p>
    <label className="mt-4 block text-sm text-zinc-300">Worker access code<input required autoCapitalize="characters" autoComplete="one-time-code" inputMode="text" value={code} onChange={(e)=>setCode(e.target.value)} placeholder="XXXX-XXXX-XXXX-XXXX" className="mt-1 min-h-12 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 font-mono uppercase tracking-wider text-white" /></label>
    <button disabled={busy} className="mt-3 min-h-12 w-full rounded-lg border border-cyan-700 px-4 font-semibold text-cyan-200 disabled:opacity-60">{busy?"Checking…":"Enter Phronesis"}</button>{message?<p role="alert" className="mt-3 text-sm text-amber-200">{message}</p>:null}
  </form>;
}
